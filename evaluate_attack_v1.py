...
with tf.device("/gpu:0"):
	...
	cleanImage = tf.placeholder(tf.float32, shape=[opt.batchSize, opt.inH, opt.inW, 3])
	pgd_max = tf.clip_by_value(cleanImage + attack_epsilon, 0.0, 1.0)
	pgd_min = tf.clip_by_value(cleanImage - attack_epsilon, 0.0, 1.0)

	clippedImage = tf.clip_by_value(inputImage, pgd_min, pgd_max)

	#spatialTransformedImage = flow_st(inputImage, flow)
	spatialTransformedImage = flow_st(clippedImage, flow)
...
def attack(sess):
	...
	target_batch = {cleanImage:target_img, inputImage: target_img, renderTrans: target_renderTrans, depthGT: target_depthGT,
	...
	while iter_ < max_iters:
		adv_batch = {cleanImage:source_img, inputImage: x_adv, renderTrans: target_renderTrans, depthGT: target_depthGT,
		...
		if opt.attack_type == 'spatial_dag':
			...
			x_adv = x_adv - alpha_inp * grad_inp_t
